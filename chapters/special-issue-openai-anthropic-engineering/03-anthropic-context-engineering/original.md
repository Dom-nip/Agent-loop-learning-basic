...
> 📄 **原文收录** | 机构：Anthropic | 作者：Anthropic 工程团队 | 发布：2025-09-29 | 检索：2026-09-12
...
![In contrast to the discrete task of writing a prompt, context engineering is iterative and the curation phase happens each time we decide what to pass](images/fig01.webp)
*In contrast to the discrete task of writing a prompt, context engineering is iterative and the curation phase happens each time we decide what to pass*
...
![At one end of the spectrum, we see brittle if-else hardcoded prompts, and at the other end we see prompts that are overly general or falsely assume sh](images/fig02.webp)
*At one end of the spectrum, we see brittle if-else hardcoded prompts, and at the other end we see prompts that are overly general or falsely assume sh*
